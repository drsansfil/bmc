...
class InformationController extends Controller
{
    ...
    public function voir_directeur() {
        $informations = directeur::first();
        if(is_null($informations)){
            $data=[
                "nom" => "Nom du directeur",
                "description" => "Message du directeur",
            ];
            directeur::insert($data);
            $informations = directeur::first();
    ...
    public function edit_directeur(Request $request){

        $information = directeur::firstOrNew([]);
        $information->nom =$request->nom;
        $information->description =$request->description;
        $information->poste =$request->poste;
        $information->facebook =$request->facebook;
        $information->linkedin =$request->linkedin;
        $information->instagram =$request->instagram;
        ...
        if ($request->hasFile('photo')) {
            ...
            // Supprimez la précédente si elle existe
            if (!empty($information->image)) {
                File::delete(public_path('uploads/' . $information->image));
            }
        ...
        if ($information->save()) {
            return redirect('/directeur')->with('message', 'Les informations ont été enregistrer !');
        } else {
            return redirect('/directeur')->with('message', 'Echec');
        }
    }
}
